...
#
from minet.web import create_pool_manager
...
class MediacloudAPIClient:
    ...
    def count(self, query, **kwargs):
        return mediacloud_search(
            self.pool_manager, self.token, query, count=True, **kwargs
        )

    def search(self, query, **kwargs):
        return mediacloud_search(
            self.pool_manager, self.token, query, count=False, **kwargs
        )

    def topic_stories(self, topic_id, **kwargs):
        return mediacloud_topic_stories(
            self.pool_manager, self.token, topic_id, **kwargs
        )
